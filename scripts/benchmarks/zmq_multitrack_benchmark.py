...
"""
ZMQ多轨道延迟基准测试脚本
任务: Task #134 - Multi-Symbol Expansion & Three-Track Support
功能: 并发测试多个交易品种(3轨或更多)的ZMQ REQ-REP延迟
...
BENCHMARK_CONFIG = {
    ...
    "zmq_req_port": 5555,
    "zmq_pub_port": 5556,
    "test_duration_seconds": 60,  # 多轨测试时间
    "min_samples": 100,  # 最少100条样本
    "max_concurrent_threads": 10,  # 最大并发线程数
}

# 交易品种 (Task #133: EURUSD.s, BTCUSD.s + Task #134新增: GBPUSD.s)
SYMBOLS = ["EURUSD.s", "BTCUSD.s", "GBPUSD.s"]

# 输出文件
RESULTS_FILE = Path("/opt/mt5-crs/zmq_multitrack_results.json")
LOG_FILE = Path("/opt/mt5-crs/TASK_134_VERIFY.log")
REPORT_FILE = Path("/opt/mt5-crs/TASK_134_CAPACITY_REPORT.md")
...
class BenchmarkLogger:
    ...
    def log(self, message: str, level: str = "INFO"):
        ...
        log_entry = f"[{timestamp}] [{level}] [Task#134-Benchmark] {message}"
        print(log_entry)
...
class MultiTrackZMQBenchmark:
    """多轨ZMQ延迟基准测试器(支持并发品种测试)"""
    ...
    def run_multitrack_benchmark(self) -> bool:
        """运行多轨基准测试(并发执行所有品种)"""
        self.logger.log("=" * 70)
        self.logger.log("启动多轨ZMQ延迟基准测试 (Task #134)")
        self.logger.log("=" * 70)
        ...
        try:
            ...
            self.logger.log(f"❌ 多轨基准测试失败: {e}", "ERROR")
            return False

    def _generate_summary(self):
        """生成汇总报告"""
        self.logger.log("\n" + "=" * 70)
        self.logger.log("多轨基准测试汇总")
        self.logger.log("=" * 70)
        ...
        if all_p99_values:
            ...
            self.logger.log(f"容量评级: {'✅ 可接受' if track_count <= 3 else '⚠️ 需评估'}")

            self.results["summary"] = {
                "total_tracks": track_count,
                "max_p99_latency": max_p99,
                "capacity_budget": capacity_budget,
                "status": "acceptable" if track_count <= 3 else "warning"
            }
...
def main():
    ...
    logger.log("=" * 70)
    logger.log("Multi-Track ZMQ Latency Benchmark (Task #134)")
    logger.log("=" * 70)
    ...
    benchmark = MultiTrackZMQBenchmark(logger)

    # 运行多轨基准测试
    success = benchmark.run_multitrack_benchmark()

    # 保存结果
    if success:
        benchmark.save_results()
        logger.log("\n[UnifiedGate] PASS - 多轨基准测试成功完成")
        sys.exit(0)
    else:
        logger.log("\n[UnifiedGate] FAIL - 多轨基准测试失败")
        sys.exit(1)
