...
**唯一主实例绝对路径**：`/root/M t 5-CRS`

**关键目录结构**：
- 项目根目录：`/root/M t 5-CRS`
- Git 仓库：`/root/M t 5-CRS/.git`
- 文档目录：`/root/M t 5-CRS/docs`
- 脚本目录：`/root/M t 5-CRS/scripts`
- 配置目录：`/root/M t 5-CRS/configs`

**强制规则**：
1. **禁止使用相对路径**或其他变体目录（Desktop、Downloads、MT5、M-t-5-CRS 等）
2. **所有文件操作**（读、写、启动、编译）必须使用上述绝对路径
3. **路径不明确或文件不存在时**，立即停止操作并报告错误，不得自行尝试其他目录
4. **已清理冗余实例**：`/root/MT5`、`/root/M-t-5-CRS` 已备份并删除（备份位置：`/root/mt5_backups/`）
