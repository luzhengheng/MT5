...
- 建议检查项:
  - [ ] 在云服务商(阿里云)侧确认VPC路由/安全组已正确配置
  - [ ] 检查 .env 和 .env.production 文件权限 (chmod 600)
  - [ ] 验证敏感配置文件未被意外暴露
...
1. **为什么要迁移?**
   - 172.19.141.255 是 /24 子网的定向广播地址 (RFC 919)
   - RFC 3021 定义的是点对点链路上使用 /31 前缀时的特殊处理，与本场景无直接关系
   - 使用广播地址作为单播端点会导致网络风暴或连接不稳定
